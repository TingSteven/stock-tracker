...
use crate::{
    models::{HistoryPoint, SectorData},
...
async fn fetch_sector_single(name: &str, symbol: &str) -> Option<SectorData> {
    ...
    Some(SectorData {
        ...
        change_3m,
        momentum_score,
        rotation_signal: rotation_signal.to_string(),
        volume_ratio,
        vs_benchmark: None,
    })
}
...
async fn fetch_sector_multi(name: &str, symbols: &[&str], bench_momentum: f64) -> Option<SectorData> {
    let futs: Vec<_> = symbols.iter().map(|&s| fetch_chart(s)).collect();
    let results = join_all(futs).await;

    let valid: Vec<(Vec<HistoryPoint>, Value)> =
...
pub async fn get_sectors(req: Request, _ctx: RouteContext<()>) -> worker::Result<Response> {
    let url = req.url()?;
    ...
    if market == "tw" {
        ...
        let futs: Vec<_> = TW_SECTORS_MULTI
            .iter()
...
pub fn parse_param(query: &str, key: &str) -> Option<String> {
